...
public class SmartWatchGridController implements Initializable {
    ...
    private List<SmartWatch> allWatches;
    ...
    private void loadData() {
        try (UnitOfWorkImpl uow = new UnitOfWorkImpl()) {
            allWatches = uow.getSmartWatchRepository().getFullAll();
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private void handleSearch() {
        ...
        List<SmartWatch> filtered = allWatches.stream()
                .filter(w -> query.isEmpty()
                        || w.getName().toLowerCase().contains(query)
                        || (w.getBrand() != null && w.getBrand().getName().toLowerCase().contains(query)))
                .filter(w -> category == null
                        || (w.getCategory() != null && w.getCategory().getName().equals(category)))
                .filter(w -> brand == null
                        || (w.getBrand() != null && w.getBrand().getName().equals(brand)))
    ...
    private VBox buildCard(SmartWatch watch) {
        ...
        // Slika ili emoji fallback — samo jedan od njih
        ...
        card.setOnMouseClicked(e -> openDetail(watch));
        ...
        if (imagePath != null && !imagePath.isEmpty()) {
            try {
                ...
                if (is != null) {
                    ...
                    return iv; // ← slika pronađena, vrati ImageView
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        // Fallback — slika nije pronađena
